# COLOCANDO BOTÕES
Adicionar botões ao seu jogo pode proporcionar uma experiência mais interativa. Vamos adicionar botões para reiniciar o jogo após o fim e para pausar e retomar o jogo durante o gameplay.
...
2. **index.js:**
   Atualize o arquivo `index.js` com o seguinte código para adicionar botões ao jogo:
   ...
   function create() {
       ...
       // Adiciona botões
       this.restartButton = this.add.text(700, 16, 'Reiniciar', { fontSize: '24px', fill: '#000' })
           .setInteractive()
           .on('pointerdown', () => restartGame.call(this));

       this.pauseButton = this.add.text(700, 50, 'Pausar', { fontSize: '24px', fill: '#000' })
           .setInteractive()
   ...
   Adicionamos dois botões (`restartButton` e `pauseButton`) à interface do usuário e suas funcionalidades. O botão de reinício reinicia o jogo, enquanto o botão de pausa pausa e retoma o jogo.

2. **Executando o Jogo:**
   Certifique-se de que o servidor local ainda está em execução (caso contrário, execute `npx http-server` novamente) e abra o navegador para `http://localhost:8080`. Agora, você pode reiniciar e pausar o jogo usando os botões adicionados.
